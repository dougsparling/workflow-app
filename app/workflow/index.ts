...
const getExchangeRate = tool(
  (input) => `1 ${input.from.toUpperCase()} = ${input.from.toUpperCase() === "JPY" ? "0.0067" : input.from.toUpperCase() === "CAD" ? "0.74" : "1.00"} USD`,
  {
...
export async function runAgent() {
  const resp = await agenticLoop([new HumanMessage("What's the weather like in Tokyo, San Francisco, and Montreal?")]);
